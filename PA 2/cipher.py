...
def main():
    ...
    for i in range(len(MSGS)):
        XORMATRIX.append([0]*len(MSGS))

    for i in range(len(MSGS)):
        for k in range(i+1,len(MSGS)):
            XORMATRIX[i][k] = strxor(MSGS[i].decode("hex"), MSGS[k].decode("hex"))
            XORMATRIX[k][i] = XORMATRIX[i][k]
            #print XORMATRIX[i][k]
    
    blanklines = []
    for i in range(len(MSGS)):
        blanklines.append([0]*300)

    for i in range(len(MSGS)):
        for k in range(len(MSGS)):
            if (XORMATRIX[i][k]!=0):
                for t in range(len(XORMATRIX[i][k])):
                    obj = re.match(r'[a-zA-Z]',XORMATRIX[i][k][t],flags=0) 
                    if obj:
                        blanklines[i][t] = blanklines[i][t]+1
                        
    KEY = ['0']*200  
    
    for i in range(len(MSGS)):
        for k in range(len(blanklines[i])):
            if blanklines[i][k] > 5: 
                KEY[k] = strxor(MSGS[i].decode('hex')[k]," ")
    print KEY
    key2 = ''.join(KEY)
